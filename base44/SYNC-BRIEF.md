...
> ⚠️ **החלטת תוכן:** `מסחרי` ו-`ביצוע` ממופים כרגע ל-`build` (=בנייה למגורים), כי באתר יש כיום רק 3 קטגוריות. זה ייצג אותם תחת התווית/הסינון "בנייה למגורים" — לא מדויק. אם תרצו, אפשר **להוסיף קטגוריה ייעודית** (למשל `commercial`=מסחרי) עם תווית וסינון משלה באתר. דורש שינוי קל בקוד האתר — ראו §5.
...
✅ מיפוי שדות (§3) ו-`category`→`cat` (§3א) — אושרו. נותרו:
1. **⛔ ערכי ה-enum של `status`** — רשימת הערכים האפשריים, כדי למלא את §3ב (→ תווית `sale` + `stage` 1/2/3).
2. **🟡 החלטת תוכן — קטגוריית "מסחרי":** למפות `מסחרי`/`ביצוע` ל-`build` (פשוט), או **להוסיף קטגוריה ייעודית `commercial`** באתר (תווית + סינון נפרדים)? אם כן — אבצע את השינוי בקוד האתר.
3. **דירות (Unit):** האם קיים entity של דירה בודדת עם שדה סטטוס (נמכר/פנוי)? אם כן — שם ה-entity, שם שדה הסטטוס וערכיו. אם לא — נדלג בינתיים על סנכרון דירות.
